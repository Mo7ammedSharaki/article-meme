...
yourpath = r"D:\my articles\make bot using python\meme"
lis = []
...
def start_commend(update: Update, context: CallbackContext):
    kd_layout = [["genrate meme"]]
    kbds = ReplyKeyboardMarkup(kd_layout)
    update.message.reply_text(
        text="""press on "genrate meme" """, reply_markup=kbds)
...
def echo(update: Update, context: CallbackContext):
    ...
    sleep(1)
    file = lis[n]
    if update.message.text == "genrate meme":
